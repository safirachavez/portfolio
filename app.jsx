...
function Conversation({ userMsg, aiResponse, thinking, onBackTopics }) {
  return (
    <div className="chat">
...
function App() {
  ...
  const [aiResponse, setAiResponse] = useState(null); // live AI response text
  ...
  // open a conversation: show the user's message, call AI, then reveal response
  const pick = async (id, userText) => {
    const entry = DATA.ENTRIES.find(e => e.id === id);
    const msg = (userText || (entry ? entry.q : "")).trim();
    setActive(id);
    setUserMsg(msg);
    setAiResponse(null);
    setSubmitted("");
    ...
    try {
      ...
      setAiResponse(data.answer || "I'm not sure about that one — feel free to email me at safirachavezw@gmail.com");
    } catch (e) {
      setAiResponse("Something went wrong on my end. Reach me at safirachavezw@gmail.com");
    } finally {
      setThinking(false);
    }
  };
  ...
  const clearAll = () => {
    ...
    setUserMsg(""); setThinking(false); setAiResponse(null);
    pendingTarget.current = null;
    requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: "smooth" }));
  };
